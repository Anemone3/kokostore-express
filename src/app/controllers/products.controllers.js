...
import { validationResult } from 'express-validator';
    ...
    const getProducts = async(req,res) =>{
        try {
            ...
            rows.forEach(row => {
                const { id, titulo, descripcion, price, category_id, name_category } = row;
            ...
            res.status(200).json({ rows: productsArray.length, products: productsArray });
    ...
    const getProductById = async(req,res) =>{
        ...
        try{
            ...
            if(!rows[0]){
                return res.status(404).json({error: `No se encuentra el producto solicitado`});
            }
...
export const productController = {
    getProducts,getProductById,getProductCategoryId
}